<template>
  <LoadingHandler :isLoading="state.loading">
    <h2
      class="viur-shop-cart-sidebar-headline headline"
      v-html="$t('viur.shop.summary_headline')"
    ></h2>
    <div class="viur-shop-cart-sidebar-summary">
      <div
        class="viur-shop-cart-sidebar-summary-item"
      ...
      >
        <template v-if="(!shopStore.state.showN1odes && item.skel_type === 'leaf') || shopStore.state.showNodes">
          <div class="viur-shop-cart-sidebar-summary-item-row">
            <div
              class="viur-shop-cart-sidebar-summary-item-amount"
              v-if="item.skel_type === 'leaf'"
            >
              {{ item.quantity }}&nbsp;&times;
            </div>
            <div
              class="viur-shop-cart-sidebar-summary-item-name"
              v-html="item.skel_type === 'node' ? item.name : item.shop_name"
            ></div>
            <div
              class="viur-shop-cart-sidebar-summary-item-price"
              v-if="getArticleDiscounts(item).length && item.price?.recommended > item.price?.current"
            >
              <sl-badge
                v-for="discount in getArticleDiscounts(item)"
                variant="danger"
                pill
              >
          ...
          <span
            class="viur-shop-cart-sidebar-summary-item-price--uvp"
            v-if="getArticleDiscounts(item).length && item.price?.recommended > item.price?.current"
          >
            UVP:
            <sl-format-number
    ...
    <template v-if="showFeatures">
      ...
      <div
        class="viur-shop-cart-sidebar-info"
        v-if="shopStore.state.cartRoot.discount && isBasketDiscount(shopStore.state.cartRoot.discount.dest.key)"
      >
    ...
    <div class="viur-shop-cart-sidebar-info"></div>
    ...
    <div
      class="viur-shop-cart-sidebar-info viur-shop-cart-shipping-item"
      v-if="shopStore.state.order?.cart?.dest?.shipping"
    >
    ...
    <slot name="action-buttons">
      <div
        ...
        v-if="false && !shopStore.state.order?.['is_paid']"
      >
        <sl-button
          variant="success"
          size="large"
          >{{ $t('shop.summary_checkout') }}</sl-button
        >
      </div>
    </slot>
  </LoadingHandler>
...
const state = reactive({
  items: computed(() => {
    return shopStore.state.cartList
  }),
  cartTotal: computed(() => {
  ...
  shippingTime: computed(() => {
    try {
      let times = shopStore.state.order?.cart?.dest?.shipping?.dest?.delivery_time_range
        .split(' - ')
        .map((x) => parseInt(x))
  ...
  discount: computed(() => {
    ...
    const cr =
      shopStore.state.cartRoot?.total != null
  ...
  total: computed(() => {
    return (
      shopStore.state.cartRoot?.total_discount_price ?? shopStore.state.order?.cart?.dest?.total_discount_price ?? 0
    )
  }),
  vat: computed(() => {
    return shopStore.state.cartRoot?.vat ?? shopStore.state.order?.cart?.dest?.vat ?? []
  }),
  sum: computed(() => {
    let sum = state.items.reduce((acc, item) => {
      if (item.skel_type === 'leaf') {
