<template>
  ...
  <loading-handler
    :isLoading="shippingState.isLoading"
    :hasError="shippingState.hasError"
    :errorMessage="shippingState.errorMessage"
  >
    <card-selector
      :options="shippingState.shippingData"
        ...
        <sl-bar class="shipping-bar">
          <div slot="left">
            <sl-format-number
              ...
              type="currency"
              currency="EUR"
              :value="option['dest']['shipping_cost']"
              v-if="option['dest']['shipping_cost']"
            >
            </sl-format-number>
...
.shipping-bar {
  width: 100%;
  margin-right: var(--ignt-spacing-large);
}
</style>
