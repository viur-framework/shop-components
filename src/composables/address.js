...
export const useAddress = defineStore('useAddressStore', () => {
      ...
      if (key === shippingAddressKey.value) {
        ...
        if (shopStore.state.cartRoot.discount) {
          shopStore.state.discounts = {
            [shopStore.state.cartRoot.discount.dest.key]: shopStore.state.cartRoot.discount,
          }
        }
      }
  ...
  function saveBirthdate(addressKey, birthdate) {
    return new Promise((resolve, reject) => {
      Request.securePost(`/json/${shopStore.state.moduleName}/address/edit/${addressKey}`, {
        dataObj: {
