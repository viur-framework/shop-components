...
export function useCart() {
  ...
  function createCart() {
    Request.post(shopStore.state.shopApiUrl + '/cart_add', {
      dataObj: {
  ...
  function fetchCartRoot() {
    ...
    return Request.get(`${shopStore.state.shopUrl}/cart/listRootNodes`).then(async (resp) => {
      ...
      shopStore.state.cartRoot = data.filter((i) => i['cart_type'] === 'basket')?.[0]
        ? data.filter((i) => i['cart_type'] === 'basket')[0]
        : []
  ...
  async function _collectCartItems(key, leafs, discounts) {
    let resp = await Request.get(`${shopStore.state.shopApiUrl}/cart_list`, {
      dataObj: {
        cart_key: key,
      },
    })
  ...
  function updateCart({
    ...
    if (commission) {
      // TODO: that's not standard, must be more generic!
  ...
  function addItem(key, quantity = 1, cart = null, quantity_mode = 'replace') {
    ...
    return Request.post(`${shopStore.state.shopApiUrl}/article_add`, {
      dataObj: {
        article_key: key,
  ...
  function removeItem(key, cart = null) {
    shopStore.state.cartIsUpdating = true
    return Request.post(`${shopStore.state.shopApiUrl}/article_remove`, {
      dataObj: {
        article_key: key,
        parent_cart_key: cart ? cart : shopStore.state.cartRoot['key'],
      },
