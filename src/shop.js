...
export const useViurShopStore = defineStore('viurshopStore', () => {
    ...
    tabs: {
      ...
      userdata: {
        ...
        active: computed(
          () =>
            state.order &&
            !state.order?.['is_checkout_in_progress'] &&
            !state.order?.['is_ordered'] &&
            state.cartList.length > 0,
        ), //active if checkout not startet and cart is not empty
        validating: false,
        valid: false,
      },
      shippingmethod: {
        ...
        active: computed(
          () =>
            !state.order?.['is_checkout_in_progress'] &&
            !state.order?.['is_ordered'] &&
            state.cartRoot?.['shipping_address'],
        ), // we need a shipping country
        validating: false,
        valid: false,
      },
      paymentprovider: {
        ...
        active: computed(
          () =>
            !state.order?.['is_checkout_in_progress'] &&
            state.order &&
            state.cartRoot?.['shipping'] &&
            state.cartRoot?.['shipping_address'],
        ), // we need a active order
        validating: false,
        valid: false,
      },
      confirm: {
      ...
      complete: {
        ...
        active: computed(
          () =>
            state.order?.['is_ordered'] &&
            (state.order?.['is_rts'] || state.order?.['is_paid'] || state.isPaymentPending),
        ), // active if ordered
        validating: false,
        valid: false,
      },
  ...
  function checkoutStart() {
    ...
    return new Promise((resolve, reject) => {
      Request.post(`${state.shopUrl}/order/checkout_start`, {
        dataObj: {
  ...
  function checkoutOrder() {
    //payment is finished
    return new Promise((resolve, reject) => {
      Request.post(`${state.shopUrl}/order/checkout_order`, {
        dataObj: {
          order_key: state.orderKey,
        },
      })
